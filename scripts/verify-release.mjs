...
const sourceAssertions = [
  ...
  ['release workflow verifier exists', 'scripts/verify-release-workflow.mjs', ['Release workflow verified', 'jobSource', 'requireJobContains', 'requireJobOrder', 'needs: verify-release', "if: github.event_name != 'pull_request'", 'production smoke command must build current shell', 'envNumber', "envNumber('PROD_SMOKE_RETRIES') < 8", "envNumber('PROD_SMOKE_RETRY_DELAY_MS') < 20000", 'production smoke job must wait before smoke command']],
