...
const sourceAssertions = [
  ...
  ['manual QA evidence validator exists', 'scripts/validate-manual-qa-evidence.mjs', ['Release commit must match current release commit', 'Android APK SHA-256 must match current repository APK', 'Fully verified must be Yes', 'Android push must be configured for full verification', 'DingTalk task message opens the task learning page', 'DingTalk challenge message opens the challenge learning page', 'Arabic DingTalk task/challenge messages render in Arabic', 'Arabic Android push notification renders in Arabic', 'Android push tap opens the correct recording/task/campaign page', 'QA row must include concrete evidence', 'Manual QA evidence validated']],
