...
const sourceAssertions = [
  ...
  ['manual QA evidence validator exists', 'scripts/validate-manual-qa-evidence.mjs', ['Release commit must match current release commit', 'Android APK SHA-256 must match current repository APK', 'Fully verified must be Yes', 'Android push must be configured for full verification', 'DingTalk task message opens the task learning page', 'DingTalk challenge message opens the challenge learning page', 'Arabic DingTalk task/challenge messages render in Arabic', 'Arabic Android push notification renders in Arabic', 'Android push tap opens the correct recording/task/campaign page', 'Manual QA evidence validated']],
  ['manual QA evidence validator verifier exists', 'scripts/verify-manual-qa-evidence-validator.mjs', ['Manual QA evidence validator verified', 'Expected invalid evidence with a Fail row to be rejected', 'Expected invalid evidence with a Not configured push row to be rejected', 'Expected invalid evidence with a stale release commit to be rejected']],
