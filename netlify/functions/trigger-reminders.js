...
if (!admin.apps.length) {
    try {
        if (process.env.FIREBASE_SERVICE_ACCOUNT) {
            ...
            console.log("Firebase Admin successfully initialized in trigger-reminders function.");
        } else {
            console.warn("FIREBASE_SERVICE_ACCOUNT env var not found. Running in mockup fallback mode.");
        }
    } catch (error) {
        console.error("Firebase Admin Initialization Error in trigger-reminders function:", error);
    }
}
...
export const handler = async (event, context) => {
    console.log("[Trigger Reminders HTTP] Manual trigger triggered at:", new Date().toISOString());
    ...
    try {
        ...
        console.warn("[Trigger Reminders HTTP] Firestore connection unavailable. Skipping scan:", dbErr.message);
    ...
    try {
        ...
        if (!isMockDingTalk) {
            try {
                ...
                if (tokenData.errcode === 0) {
                    ...
                    console.error("[Trigger Reminders HTTP] Failed to exchange DingTalk token:", tokenData.errmsg);
                    diagnostics.errors.push(`DingTalk token exchange error: [${tokenData.errcode}] ${tokenData.errmsg}`);
                }
            } catch (tokenErr) {
                console.error("[Trigger Reminders HTTP] DingTalk token request network error:", tokenErr.message);
        ...
        console.log(`[Trigger Reminders HTTP] Scanning ${snapshot.size} active learning tasks...`);
        ...
        for (const doc of snapshot.docs) {
            ...
            try {
                ...
                console.error(`[Trigger Reminders HTTP] Failed to parse deadline for task ID ${task.id}:`, parseErr.message);
                diagnostics.scannedTasks.push({
                    id: task.id,
                    title: task.title || 'Untitled',
            ...
            console.log(`[Trigger Reminders HTTP] Task "${task.title}" (ID: ${task.id}) is due for ${timeLabel} warning (Deadline: ${deadlineDate.toLocaleString()}).`);
            ...
            if (pendingAssigneeIds.length === 0) {
                console.log(`[Trigger Reminders HTTP] All assignees have completed Task "${task.title}". Marking ${reminderField} as true.`);
            ...
            if (dingtalkUserIds.length === 0) {
                console.log(`[Trigger Reminders HTTP] No linked DingTalk User IDs found for pending assignees of Task "${task.title}". Marking ${reminderField} as true.`);
            ...
            if (accessToken && agentId) {
                try {
                    ...
                    if (notifyData.errcode === 0) {
                        sentSuccess = true;
                        console.log(`[Trigger Reminders HTTP] DingTalk Corp Message sent successfully for Task "${task.title}" to: ${dingtalkUserIds.join(',')}`);
                        taskDiagnostic.action = `Notification sent successfully to ${dingtalkUserIds.join(',')}`;
                    } else {
                        console.error(`[Trigger Reminders HTTP] DingTalk message send failed: [${notifyData.errcode}] ${notifyData.errmsg}`);
                        apiErrMessage = `DingTalk send failed: [${notifyData.errcode}] ${notifyData.errmsg}`;
                        taskDiagnostic.action = `Error: ${apiErrMessage}`;
                    }
                } catch (err) {
                    console.error(`[Trigger Reminders HTTP] DingTalk message API query network exception:`, err.message);
                ...
                sentSuccess = true;
                console.log(`[Mock Trigger Reminders HTTP] Triggered ${timeLabel} warning for Task "${task.title}" (Mock recipients: ${dingtalkUserIds.join(',')})`);
        ...
        console.error("[Trigger Reminders HTTP] Job encountered error:", err);
